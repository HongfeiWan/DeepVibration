...
def plot_cumulative_cut_spectra(
    ...
    *,
    ...
    for i, (name, step_mask) in enumerate(cut_steps):
        ...
        # 从 ACT（或 basic_cut/cut_time）开始以及之后的所有 step，统一使用 reduced exposure
        if name in ("ACT", "basic_cut", "cut_time"):
        ...
        # 第一条曲线用自身 name 作为图例；第二条 basic 仍按 basic 命名更直观
        if i == 0 or name == "basic":
            label = f"{name} (N={n_pass})"
        else:
            label = f"above+{name} (N={n_pass})"
...
def cut_pncut(
    ...
    max_ch0: np.ndarray,
    max_ch1: np.ndarray,
    fit_ch0_min: float = 5000.0,
    fit_ch0_max: float = 16000.0,
    n_sigma: float = 0.8,
...
def cut_bscut(
    ...
    *,
    ...
    return (
        np.isfinite(rise_us)
        & (rise_us >= float(rise_time_min_us))
...
if __name__ == "__main__":
    ...
    m_ch0_sat = cut_ch0_max_saturation(max_ch0_all)
    ...
    # 与 efficiency.py 的时间 cut 形式对齐：
    # 使用 ~m_acv 侧（ACT=~ACV）事件集合作为坏时间段统计的 pre_mask。
    base_after_pre = (
        ~m_acv
        & m_fit_ok
        & m_ch0_min
    ...
    # 与 efficiency.py 对齐：仅保留两步
    # - 第 1 步：ACT（~acv_mask）事件集合作为基础点集
    # - 第 2 步：在 ACT 集合上叠加 pncut
    act_mask = (
        ~m_acv
        & m_ch0_time
        & m_ch0_min
    ...
    m_pn = cut_pncut(act_mask, max_ch0_all, max_ch1_all)
