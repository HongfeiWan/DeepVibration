...
# -*- coding: utf-8 -*-
"""
PN-cut 选中事件的 CH0 波形 8 参数 UMAP + PN-cut 不正常事例筛选脚本。
...
4. 使用 UMAP 降维到二维；
5. 筛选两类不正常事例：A) UMAP1<6 且 max_ch0<1500；B) UMAP1≥6 且 max_ch0>1500；
...
from analysis.features.filters import median_filter
...
def run_umap_pncut_visualization(
    ...
    左右并排绘制 UMAP 图与 PN-cut 图，A/B 组用方框/三角标注；并分别绘制 A、B 组的 CH0 波形。
    """
    ...
    # A) UMAP1<6 且 max_ch0<1500；B) UMAP1≥6 且 max_ch0>1500
    ...
    # 5. 绘图：UMAP + PN-cut 双图（参考 ch0_umap&pncut_select_abnormal.py）
    ...
    ax_umap.scatter(
        ...
        label=f"A: UMAP1<{umap1_threshold} & max_ch0<{max_ch0_threshold} (n={n_A})", zorder=5,
    )
    ax_umap.scatter(
        embedding[mask_B, 0], embedding[mask_B, 1],
        c="lime", s=80, alpha=0.9, marker="^", edgecolors="black", linewidths=1,
        label=f"B: max_ch0>{max_ch0_threshold} & UMAP1≥{umap1_threshold} (n={n_B})", zorder=5,
    )
    ...
    ax_pncut.set_title(
        f"PN-cut: max CH0 vs max CH1 | A/B 异常组 (n_umap1_lt={n_umap1_lt})",
    ...
    # 6. 多窗口：分别显示组 A、组 B 的全部 CH0 原始波形
    ...
    if n_A > 0:
        ...
        ax_wf_A.set_title(f"组 A: UMAP1<{umap1_threshold} & max_ch0<{max_ch0_threshold} (n={n_A})", fontsize=11, fontweight="bold")
    else:
    ...
    if n_B > 0:
        ...
        ax_wf_B.set_title(f"组 B: max_ch0>{max_ch0_threshold} & UMAP1≥{umap1_threshold} (n={n_B})", fontsize=11, fontweight="bold")
    else:
    ...
    if save_path is None:
        ...
        save_path = os.path.join(output_dir, f"ch0_8param_umap_pncut_abnormal_{n_events}events_{timestamp}.png")

    fig.savefig(save_path, dpi=300, bbox_inches="tight", facecolor="white")
    print(f"8 参数 UMAP+PN-cut 不正常事例可视化已保存至: {save_path}")
