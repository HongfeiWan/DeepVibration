...
# -*- coding: utf-8 -*-
"""
PN-cut 选中事件的 CH0 波形 8 参数 UMAP 降维可视化脚本。
...
def run_umap_visualization(
    ...
    """
    对 PN-cut ±1σ 带内事件的 CH0 波形，并行提取 8 参数 → StandardScaler → UMAP 2D → 可视化。
    """
    ...
    # UMAP 降维到 3D
    reducer = umap.UMAP(n_components=3, n_neighbors=umap_n_neighbors, min_dist=umap_min_dist, random_state=42)
    ...
    ax = fig.add_axes([0.08, 0.12, 0.62, 0.82], projection="3d")
    ...
    if save_path is None:
        ...
        save_path = os.path.join(output_dir, f"ch0_8param_umap3d_{n_events}events_{timestamp}.png")
