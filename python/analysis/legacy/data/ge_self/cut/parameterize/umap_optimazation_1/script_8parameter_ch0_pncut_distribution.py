...
def run_umap_pncut_visualization(
    ...
    2）分别对 8 个参数，在 UMAP1 阈值左右两侧样本上绘制分布直方图，用于对比。
    """
    ...
    fig_params, axes = plt.subplots(2, 4, figsize=(16, 8))
    axes = axes.ravel()

    for i, name in enumerate(PARAM_NAMES):
        ax = axes[i]
        values = params_dict[name]
        values_left = values[umap1_lt]
        values_right = values[~umap1_lt]
    ...
    # 当 PARAM_NAMES 少于子图数量时，删除多余坐标轴（目前为 8 个参数，正好填满）
    if len(PARAM_NAMES) < len(axes):
        for j in range(len(PARAM_NAMES), len(axes)):
            fig_params.delaxes(axes[j])

    fig_params.suptitle(
        f"CH0 8 Parameters Distribution | UMAP1 Threshold = {umap1_threshold}",
        fontsize=13,
        fontweight="bold",
    )
    ...
    save_path_params = save_path.replace(
        "ch0_8param_umap_pncut_", "ch0_8param_distributions_"
    )
    ...
    print(f"8 参数在 UMAP1 阈值左右的分布图已保存至: {save_path_params}")
