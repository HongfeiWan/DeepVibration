...
def run_umap_visualization(
    ...
    if feature_weights is not None:
        ...
        if weights.shape[0] != X_scaled.shape[1]:
            raise ValueError(f"feature_weights 长度应为 {X_scaled.shape[1]}，当前为 {weights.shape[0]}")
    ...
    if save_path is None:
        ...
        save_path = os.path.join(output_dir, f"ch0_8param_umap_{n_events}events_{timestamp}.png")
...
if __name__ == "__main__":
    try:
        run_umap_visualization(
            ...
            umap_n_neighbors=10,
            umap_min_dist=0.1,
            feature_weights=[2.0, 1.0, 1.0, 1.0, 1.0, 2.0, 1.0, 1.0],
            save_path=None,
            show_plot=True,
