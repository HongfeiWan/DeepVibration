...
# -*- coding: utf-8 -*-
"""
PN-cut 选中事件的 CH0+CH3 波形 15 参数 UMAP 降维可视化脚本。

流程：
1. 使用 PN-cut 选出 ±1σ 带内的事件；
2. 提取 CH0 的 10 参数 + CH3 快放 5 参数（rise_t, famp, fslope, fcross, fped）；
3. StandardScaler 标准化 → UMAP 2D；
4. 可视化，着色可切换 15 个参数。
快放参数：tanh 拟合 f(x)=0.5*p0*tanh(p1*(x-p2))+p3，rise_t=ln(19)/p1。
"""
...
def _compute_fast_params_one(
    ...
    """
    对单条波形做 tanh 拟合，返回 (rise_t, famp, fslope, fcross, fped) 或 None。
    rise_t = ln(19) / p1
    """
...
def _compute_fast_params_all(
    ...
    with h5py.File(ch0_3_file, "r") as f:
        ...
        waveforms = np.stack(
            [channel_data[:, ch3_idx, idx].astype(np.float64) for idx in global_indices],
            axis=0,
        )
    n_wf = waveforms.shape[0]
    tasks = [
        (waveforms[i], baseline_window_us, smooth_window, smooth_times)
        for i in range(n_wf)
    ]
    results = []
    with ProcessPoolExecutor(max_workers=max_workers) as exe:
        results = list(exe.map(_fast_params_worker, tasks))
...
def run_umap_visualization(
    ...
    """
    对 PN-cut ±1σ 带内事件的 CH0+CH3 波形，提取 15 参数 → StandardScaler → UMAP 2D → 可视化。

    15 参数：CH0 的 Amax, Amin, Tmax, Tmin, Q, Qpre, Qprev, ped, pedt, abs(pedt-ped)；
    CH3 快放：rise_t=ln(19)/p1, famp=p0, fslope=p1, fcross=p2, fped=p3。
    ...
    # 1. 选出 ±1σ 带内的事件（始终需要，用于 fast params 或校验）
    ...
    if use_cached_params and os.path.isfile(cache_path):
        ...
        if X.shape[0] != n_events:
            raise ValueError(f"缓存事件数 {X.shape[0]} 与 ±1σ 事件数 {n_events} 不一致")
        ...
        print(f"参数矩阵形状: {X.shape} (n_events × {X.shape[1]})")
    else:
        print(f"\n将对 {n_events} 个事件进行 CH0+CH3 参数并行提取...")
        ch0_dict = {}
        fast_dict = {}
        ...
        X = np.column_stack([params_dict[name] for name in param_names])
        print(f"参数矩阵形状: {X.shape} (n_events × 15)")
        _save_params_cache(cache_path, params_dict, param_names)
    ...
    ax.set_title(
        f"CH0+CH3 Waveform 15-Parameter UMAP\n"
    ...
    if save_path is None:
        ...
        save_path = os.path.join(output_dir, f"ch0_ch3_umap_{n_events}events_{timestamp}.png")
...
if __name__ == "__main__":
    try:
        run_umap_visualization(
            ...
            params_cache_path=None,   # None 时使用 ch0_ch3_params_cache.h5
        )
