...
# -*- coding: utf-8 -*-
"""
PN-cut 选中事件的 CH0 波形 2 参数（Amax, Tmax）UMAP 降维可视化脚本。

流程：
1. 使用 PN-cut 选出 ±1σ 带内的事件；
2. 使用所有 CPU 并行处理所有波形，得到 n×2 参数矩阵（Amax, Tmax）；
3. 对 2 个参数使用 StandardScaler 标准化；
4. 使用 UMAP 降维到二维；
5. 可视化二维散点图，并可在 Amax 与 Tmax 之间切换着色。

2 参数（来自 parameter(ch0).py）：
Amax, Tmax
"""
...
# 参数缓存文件默认路径（脚本同目录）
DEFAULT_PARAMS_CACHE_PATH = os.path.join(current_dir, "ch0_2params_cache.h5")

# 只使用 2 个参数：Amax, Tmax
PARAM_NAMES = ["Amax", "Tmax"]
PARAM_KEYS = ["amax", "tmax_us"]
...
def compute_eight_params_for_events(
    ...
    """对选中的事件计算所需的 2 个参数（Amax, Tmax）（多进程并行）。"""
    print("\n正在计算所有事件的 2 个参数 (Amax, Tmax)...")
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        with tqdm(total=n_events, desc="计算2参数(Amax,Tmax)", unit="事件", ncols=100) as pbar:
...
def run_umap_visualization(
    ...
    """
    对 PN-cut ±1σ 带内事件的 CH0 波形，并行提取 2 个参数（Amax, Tmax）→ StandardScaler → UMAP 2D → 可视化。
    """
    ...
    if use_cached_params and os.path.isfile(cache_path):
        ...
        print(f"参数矩阵形状: {X.shape} (n_events × {n_features})")
    else:
        ...
        n_events = event_ranks.size
        print(f"\n将对 {n_events} 个事件进行 2 参数 (Amax, Tmax) 并行提取...")
    ...
    ax.set_title(
        f"CH0 2-Parameter UMAP (Amax, Tmax)\n"
    ...
    if save_path is None:
        ...
        save_path = os.path.join(output_dir, f"ch0_2param_umap_{n_events}events_{timestamp}.png")
