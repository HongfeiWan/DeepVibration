...
def read_vibration_data(file_path: str, downsample_factor: int = 1) -> pd.DataFrame:
    ...
    try:
        with h5py.File(file_path, 'r') as h5f:
            ...
            for col_name in acceleration_columns:
                if col_name in h5f:
                    if downsample_factor > 1:
...
def select_by_date_range_vibration(data_dir: str,
    ...
    print(f'\n开始读取 {len(detector_list)} 个探测器的加速度数据: {detector_list}')
    if downsample_factor > 1:
...
def plot_acceleration_data(data_dict: Dict[str, np.ndarray],
                       save_path: Optional[str] = None,
                       show_plot: bool = True,
                       figsize: Tuple[int, int] = (14, 8)) -> None:
    """
    绘制加速度数据随时间的变化
    如果数据中包含多个探测器（detector_num列），会为每个加速度分量创建子图，每个子图显示所有探测器
    
    参数:
        data_dict: 包含加速度数据的字典，必须包含 'datetime' 和加速度列
    ...
    # 检查是否有加速度数据
    available_accel_cols = [col for col in acceleration_columns if col in data_dict]
    if not available_accel_cols:
        print('警告：没有找到加速度数据列，无法绘图')
        return
    ...
    accel_colors = {'Acceleration_x': '#2E86AB', 'Acceleration_y': '#A23B72', 'Acceleration_z': '#F18F01'}
    
    # 创建图形和子图 - 为每个加速度分量创建子图
    fig, axes = plt.subplots(len(available_accel_cols), 1, figsize=figsize, sharex=True)
    if len(available_accel_cols) == 1:
        axes = [axes]
    
    # 获取时间范围用于日期格式化
    all_datetime_valid = []
    for accel_col in available_accel_cols:
        accel_data = data_dict[accel_col]
        if has_multiple_detectors:
    ...
    # 为每个加速度分量绘制子图
    for ax_idx, accel_col in enumerate(available_accel_cols):
        ax = axes[ax_idx]
        accel_data = data_dict[accel_col]
        ...
        if has_multiple_detectors:
            ...
            for i, det_num in enumerate(sorted(unique_detectors)):
                ...
                det_accel = accel_data[det_mask]
                
                # 过滤NaN值
                valid_mask = ~np.isnan(det_accel)
                det_datetime_valid = det_datetime[valid_mask]
                det_accel_valid = det_accel[valid_mask]
                
                if len(det_datetime_valid) > 0:
                    color = colors[i % len(colors)]
                    ax.plot(det_datetime_valid, det_accel_valid,
                           linewidth=1.5,
                           alpha=0.85,
                           color=color,
                    ...
                    # 计算统计信息
                    mean_val = np.mean(det_accel_valid)
                    std_val = np.std(det_accel_valid)
                    min_val = np.min(det_accel_valid)
                    max_val = np.max(det_accel_valid)
                    all_stats.append((det_num, len(det_accel_valid), min_val, max_val, mean_val, std_val))
        else:
            # 单个探测器：绘制单条曲线
            valid_mask = ~np.isnan(accel_data)
            datetime_valid = datetime_arr[valid_mask]
            accel_valid = accel_data[valid_mask]
            
            if len(datetime_valid) > 0:
                color = accel_colors.get(accel_col, colors[0])
                label = accel_col
                ax.plot(datetime_valid, accel_valid,
                       linewidth=1.5,
                       alpha=0.85,
                       color=color,
                       label=label)
                ...
                # 计算统计信息
                mean_val = np.mean(accel_valid)
                std_val = np.std(accel_valid)
                min_val = np.min(accel_valid)
                max_val = np.max(accel_valid)
                all_stats.append((accel_col, len(accel_valid), min_val, max_val, mean_val, std_val))
        
        # 设置标签
        ax.set_ylabel(f'{accel_col} (g)', fontsize=13, fontweight='normal')
        if ax_idx == len(available_accel_cols) - 1:
        ...
        if len(all_stats) > 0:
            ...
            for _, _, min_val, max_val, _, _ in all_stats:
                all_accel_vals.extend([min_val, max_val])
            accel_range = np.max(all_accel_vals) - np.min(all_accel_vals)
            if accel_range > 0:
                minor_interval = accel_range / 20
            ...
            for stat_item in all_stats:
                if has_multiple_detectors:
                    ...
                    stats_text += (f'Det {det_num}: N={n}, '
                                 f'Mean={mean_val:.2f} g, '
                                 f'Std={std_val:.2f} g\n')
                else:
                    accel_name, n, min_val, max_val, mean_val, std_val = stat_item
                    stats_text += (f'N={n}, '
                                 f'Mean={mean_val:.2f} g, '
                                 f'Std={std_val:.2f} g\n')
...
if __name__ == '__main__':
    # 获取项目根目录
    # 从 python/data/sensor/vibration/accelerate/select.py 向上5层到达项目根目录
    # accelerate -> vibration -> sensor -> data -> python -> DeepVibration (项目根目录)
    current_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(current_dir)))))
    data_dir = os.path.join(project_root, 'data', 'vibration', 'hdf5')
    
    print('=' * 70)
    print('振动传感器加速度数据读取和筛选示例')
    print('=' * 70)
    ...
    try:
        ...
        if data_multi:
            ...
            print('\n加速度数据统计信息:')
            print('-' * 70)
            for accel_col in ['Acceleration_x', 'Acceleration_y', 'Acceleration_z']:
                if accel_col in data_multi:
                    accel_data = data_multi[accel_col]
                    valid_data = accel_data[~np.isnan(accel_data)]
                    if len(valid_data) > 0:
                        print(f'{accel_col}:')
                        ...
                        print(f'{accel_col}: 无有效数据')
            
            # 绘制加速度数据图像
            print('\n开始绘制加速度数据图像...')
            plot_acceleration_data(data_multi, save_path=None, show_plot=True)
        else:
            print('警告：未能读取到任何数据')
