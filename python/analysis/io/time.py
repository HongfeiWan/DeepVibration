...
def read_bin_time_span(bin_file_path: str, 
    ...
    try:
        ...
        start_time = epoch_start + timedelta(seconds=float(first_eventtime))
...
def process_bin_files_time_span(bin_dir: str,
    ...
    try:
        ...
        raise ImportError("Install the 'environment' extra to build time-span DataFrames") from exc
    run_numbers = []
    start_times = []
    end_times = []
...
if __name__ == '__main__':
    ...
    # 获取项目根目录
    current_dir = os.path.dirname(os.path.abspath(__file__))  # python/analysis/io
