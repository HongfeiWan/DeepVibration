...
# -*- coding: utf-8 -*-
"""
对 raw_pulse/RTCH0max 中的所有 HDF5 文件：
1. 利用所有 CPU 并行读取每个文件的 rt_ch0max；
2. 汇总所有文件中 RT 事件的 CH0max 值；
...
def _discover_rtch0max_dir_relative_to_script() -> Path:
    """
    返回 RTCH0max 目录的绝对路径。

    目录结构假定为（与 randomtrigger/ch0max.py 保持一致）：
        project_root/
          data/
            hdf5/
              raw_pulse/
...
def _read_rt_ch0max_from_file(path: Path) -> np.ndarray:
    """
    从单个 RTCH0max 文件读取 rt_ch0max 数组。
    若格式不符合预期，则返回长度为 0 的数组。
    """
    try:
        with h5py.File(path, "r") as f:
            if "rt_ch0max" not in f:
                print(f"[警告] {path.name}: 缺少数据集 'rt_ch0max'，跳过。")
                return np.empty((0,), dtype=np.float64)
            dset = f["rt_ch0max"]
            if dset.ndim != 1:
                print(
                    f"[警告] {path.name}: 'rt_ch0max' 维度不是 1, shape={dset.shape}，跳过。"
                )
                return np.empty((0,), dtype=np.float64)
...
def _worker_read_files(paths: Iterable[Path]) -> np.ndarray:
    """worker 进程：读取若干文件的 rt_ch0max 并在本进程内拼接。"""
    vals: List[np.ndarray] = []
    for p in paths:
        data = _read_rt_ch0max_from_file(p)
        if data.size > 0:
            vals.append(data)
    if not vals:
        return np.empty((0,), dtype=np.float64)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "从 raw_pulse/RTCH0max 目录中读取所有 HDF5 文件的 rt_ch0max，"
            "汇总并绘制 CH0max 的分布直方图。"
        )
    )
    parser.add_argument(
...
def main() -> None:
    ...
    rtch0max_dir = _discover_rtch0max_dir_relative_to_script()
    print(f"RTCH0max 目录: {rtch0max_dir}")

    files = _list_h5_files(rtch0max_dir)
    if not files:
        print("未在 RTCH0max 目录中找到任何 HDF5 文件。")
        return
    ...
    if not all_values_list:
        print("未从任何 RTCH0max 文件中读取到 rt_ch0max 数据。")
        return

    all_values = np.concatenate(all_values_list, axis=0)
    print(f"总 RT 事件数: {all_values.size}")
    ...
    ax.set_xlabel("RT CH0max", fontsize=12)
    ax.set_ylabel("Count", fontsize=12)
    ax.set_title(
        f"Distribution of RT CH0max (N={all_values.size})",
        fontsize=13,
    )
