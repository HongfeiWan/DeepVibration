...
# -*- coding: utf-8 -*-
"""
GMM pn cut component 1和component 2事件计数率对比绘制脚本
从所有HDF5文件中提取component 1和component 2的事件触发时间，
并绘制它们的计数率对比图
"""
import os
import sys
...
def _get_file_time_range_worker(args):
    """获取单个文件的时间范围（用于并行处理的工作函数，必须在模块级别）"""
    ch0_3_file, epoch_offset = args
    try:
        file_event_times = read_hdf5_event_times(ch0_3_file, epoch_offset)
        if len(file_event_times) > 0:
            return (ch0_3_file, file_event_times[0], file_event_times[-1])
        else:
            return (ch0_3_file, None, None)
    except Exception as e:
        return (ch0_3_file, None, None)


def _process_single_file_for_component1_and_component2_worker(args):
    """处理单个文件，返回component 1和component 2事件的时间bin计数（用于并行处理，必须在模块级别）"""
    (ch0_3_file, ch5_file, gmm_means, gmm_covariances, gmm_weights, gmm_precisions_cholesky,
     region1_label, region2_label, a, b, sigma, start_datetime, end_datetime, time_bin_size, epoch_offset) = args
    
    try:
        # 重建GMM对象
        from sklearn.mixture import GaussianMixture
        gmm = GaussianMixture(n_components=2, covariance_type="full", random_state=0)
        gmm.means_ = np.array(gmm_means)
        gmm.covariances_ = np.array(gmm_covariances)
        gmm.weights_ = np.array(gmm_weights)
        gmm.precisions_cholesky_ = np.array(gmm_precisions_cholesky)
        gmm.n_components = 2
        ...
        if physical_count == 0:
            return (os.path.basename(ch0_3_file), {}, {}, 0, 0)
        ...
        # 找出component 1和component 2的事件
        file_comp1_mask_in_sel = file_labels == region2_label
        file_comp2_mask_in_sel = file_labels == region1_label
        
        file_comp1_sel_indices = file_sel_indices[file_comp1_mask_in_sel]
        file_comp2_sel_indices = file_sel_indices[file_comp2_mask_in_sel]
        
        # 处理component 1事件
        file_time_bin_counts1 = {}
        if len(file_comp1_sel_indices) > 0:
            with h5py.File(ch0_3_file, "r") as f:
                ...
                # 转换为datetime
                epoch_start = datetime(1970, 1, 1)
                file_event_datetimes1 = []
                for time_val in file_comp1_times:
                    eventtime = time_val - epoch_offset
                    event_datetime = epoch_start + pd.Timedelta(seconds=eventtime)
                    file_event_datetimes1.append(event_datetime)
                
                # 如果指定了日期范围，先筛选
                if start_datetime is not None or end_datetime is not None:
                    file_event_datetimes1 = pd.to_datetime(file_event_datetimes1).values
                    mask = np.ones(len(file_event_datetimes1), dtype=bool)
                    if start_datetime is not None:
                        mask = mask & (file_event_datetimes1 >= start_datetime)
                    if end_datetime is not None:
                        mask = mask & (file_event_datetimes1 <= end_datetime)
                    file_event_datetimes1 = file_event_datetimes1[mask]
                else:
                    file_event_datetimes1 = pd.to_datetime(file_event_datetimes1).values
                
                # 累加到时间bin
                if len(file_event_datetimes1) > 0:
                    df_temp = pd.DataFrame({'datetime': file_event_datetimes1})
                    df_temp.set_index('datetime', inplace=True)
                    df_temp['count'] = 1
                    resampled = df_temp.resample(time_bin_size).count()
                    
                    for bin_time, count in zip(resampled.index, resampled['count'].values):
                        bin_timestamp = pd.Timestamp(bin_time).value
                        file_time_bin_counts1[bin_timestamp] = int(count)
        
        # 处理component 2事件
        file_time_bin_counts2 = {}
        if len(file_comp2_sel_indices) > 0:
            with h5py.File(ch0_3_file, "r") as f:
                time_data = f["time_data"][:]
                file_comp2_times = time_data[file_comp2_sel_indices]
                
                # 转换为datetime
                epoch_start = datetime(1970, 1, 1)
                file_event_datetimes2 = []
                for time_val in file_comp2_times:
                    eventtime = time_val - epoch_offset
                    event_datetime = epoch_start + pd.Timedelta(seconds=eventtime)
                    file_event_datetimes2.append(event_datetime)
                
                # 如果指定了日期范围，先筛选
                if start_datetime is not None or end_datetime is not None:
                    file_event_datetimes2 = pd.to_datetime(file_event_datetimes2).values
                    mask = np.ones(len(file_event_datetimes2), dtype=bool)
                    if start_datetime is not None:
                        mask = mask & (file_event_datetimes2 >= start_datetime)
                    if end_datetime is not None:
                        mask = mask & (file_event_datetimes2 <= end_datetime)
                    file_event_datetimes2 = file_event_datetimes2[mask]
                else:
                    file_event_datetimes2 = pd.to_datetime(file_event_datetimes2).values
                
                # 累加到时间bin
                if len(file_event_datetimes2) > 0:
                    df_temp = pd.DataFrame({'datetime': file_event_datetimes2})
                    df_temp.set_index('datetime', inplace=True)
                    df_temp['count'] = 1
                    resampled = df_temp.resample(time_bin_size).count()
                    ...
                    for bin_time, count in zip(resampled.index, resampled['count'].values):
                        bin_timestamp = pd.Timestamp(bin_time).value
                        file_time_bin_counts2[bin_timestamp] = int(count)
        
        return (os.path.basename(ch0_3_file), file_time_bin_counts1, file_time_bin_counts2, 
                len(file_comp1_sel_indices), len(file_comp2_sel_indices))
...
def get_component1_and_component2_event_times(
    ...
    """
    从所有HDF5文件中获取component 1和component 2的事件触发时间，并直接累加到时间bin
    ...
    返回:
        (bin_centers, counts1, counts2): bin中心时间数组、component1计数数组、component2计数数组
    """
    from sklearn.mixture import GaussianMixture

    print("=" * 70)
    print("获取GMM component 1和component 2事件触发时间（从所有HDF5文件）")
    print("=" * 70)
    ...
    # 如果指定了日期范围，先批量读取所有文件的时间范围进行筛选
    start_datetime = None
    end_datetime = None
    
    if start_date is not None or end_date is not None:
        print(f"\n步骤0: 按日期范围筛选文件: {start_date} 到 {end_date}")
        ...
        # 批量读取所有文件的时间范围（并行）
        print("  批量读取所有文件的时间范围（并行处理）...")
        
        # 使用多进程并行读取文件时间范围
        num_workers = min(mp.cpu_count(), len(matched_pairs))
        file_time_ranges = []
        error_count = 0
        
        # 准备参数
        worker_args = [(ch0_3_file, epoch_offset) for ch0_3_file, ch5_file in matched_pairs]
        
        try:
            with ProcessPoolExecutor(max_workers=num_workers) as executor:
                futures = {executor.submit(_get_file_time_range_worker, args): args[0] 
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if result[1] is not None and result[2] is not None:
                            file_time_ranges.append(result)
                    except Exception as e:
                        error_count += 1
                        if error_count <= 5:  # 只显示前5个错误
                            print(f"    警告：读取文件 {os.path.basename(futures[future])} 时间范围时出错: {e}")
                
                if completed % 50 == 0 or completed == len(matched_pairs):
                    print(f"    已处理 {completed}/{len(matched_pairs)} 个文件，成功读取 {len(file_time_ranges)} 个文件的时间范围")
        except Exception as e:
            print(f"    并行处理失败: {e}，回退到串行处理...")
            # 回退到串行处理
            for ch0_3_file, ch5_file in matched_pairs:
                try:
                    file_event_times = read_hdf5_event_times(ch0_3_file, epoch_offset)
                    if len(file_event_times) > 0:
                        file_time_ranges.append((ch0_3_file, file_event_times[0], file_event_times[-1]))
                except Exception as e2:
                    error_count += 1
                    if error_count <= 5:
                        print(f"    警告：读取文件 {os.path.basename(ch0_3_file)} 时间范围时出错: {e2}")
        
        print(f"  成功读取 {len(file_time_ranges)}/{len(matched_pairs)} 个文件的时间范围")
        
        # 筛选出需要处理的文件对
        filtered_pairs = []
        print(f"\n  筛选条件:")
        if start_datetime is not None:
            print(f"    起始时间: {start_datetime}")
        if end_datetime is not None:
            print(f"    结束时间: {end_datetime}")
        
        print(f"\n  文件时间范围信息（前10个文件）:")
        for idx, (ch0_3_file, ch5_file) in enumerate(matched_pairs[:10]):
            file_info = next((info for info in file_time_ranges if info[0] == ch0_3_file), None)
            if file_info is not None:
                file_start, file_end = file_info[1], file_info[2]
                print(f"    [{idx+1}] {os.path.basename(ch0_3_file)}: {file_start} 到 {file_end}")
        ...
        for ch0_3_file, ch5_file in matched_pairs:
            ...
            if has_overlap:
                filtered_pairs.append((ch0_3_file, ch5_file))
        
        matched_pairs = filtered_pairs
        print(f"\n  筛选后剩余 {len(matched_pairs)} 个文件对需要处理")
        
        if len(matched_pairs) == 0:
            # 提供更详细的错误信息
            print(f"\n  错误详情:")
            print(f"    指定时间范围: {start_datetime} 到 {end_datetime}")
            if len(file_time_ranges) > 0:
                print(f"    找到 {len(file_time_ranges)} 个文件的时间范围信息")
                print(f"    前5个文件的时间范围:")
                for idx, (fname, fstart, fend) in enumerate(file_time_ranges[:5]):
                    print(f"      [{idx+1}] {os.path.basename(fname)}: {fstart} 到 {fend}")
            else:
                print(f"    未能读取到任何文件的时间范围信息")
            raise ValueError("没有文件的时间范围与指定日期范围重叠")
    ...
    region1_label = int(order[0])  # component 0
    ...
    # 步骤3: 并行处理所有文件，使用训练好的GMM模型筛选component 1和component 2事件
    print("\n步骤3: 并行处理所有文件，筛选component 1和component 2事件并累加到时间bin...")
    print(f"  使用 {min(mp.cpu_count(), len(matched_pairs))} 个CPU核心进行并行处理")
    ...
    # 使用多进程并行处理
    time_bin_counts1 = {}  # component 1
    time_bin_counts2 = {}  # component 2
    processed_file_count = 0
    num_workers = min(mp.cpu_count(), len(matched_pairs))
    
    process_args = [
        (ch0_3_file, ch5_file, gmm_means, gmm_covariances, gmm_weights, gmm_precisions_cholesky,
         region1_label, region2_label, a, b, sigma, start_datetime, end_datetime, time_bin_size, epoch_offset)
        for ch0_3_file, ch5_file in matched_pairs
    ]
    
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        futures = {executor.submit(_process_single_file_for_component1_and_component2_worker, args): args[0] 
        ...
        for future in as_completed(futures):
            ...
            try:
                result_filename, file_time_bin_counts1, file_time_bin_counts2, comp1_count, comp2_count = future.result()
                
                # 累加到总计数
                for bin_timestamp, count in file_time_bin_counts1.items():
                    if bin_timestamp not in time_bin_counts1:
                        time_bin_counts1[bin_timestamp] = 0
                    time_bin_counts1[bin_timestamp] += count
                
                for bin_timestamp, count in file_time_bin_counts2.items():
                    if bin_timestamp not in time_bin_counts2:
                        time_bin_counts2[bin_timestamp] = 0
                    time_bin_counts2[bin_timestamp] += count
                
                if comp1_count > 0 or comp2_count > 0:
                    processed_file_count += 1
                    print(f"  [{completed}/{len(matched_pairs)}] {result_filename}: "
                          f"找到 {comp1_count} 个 component 1 事件，{comp2_count} 个 component 2 事件")
                else:
                    print(f"  [{completed}/{len(matched_pairs)}] {result_filename}: 跳过（无component事件）")
                    
            except Exception as e:
                print(f"  [{completed}/{len(matched_pairs)}] {filename}: 错误 - {e}")
                import traceback
                traceback.print_exc()

    if len(time_bin_counts1) == 0 and len(time_bin_counts2) == 0:
        raise RuntimeError("未能从任何文件中读取到 component 1 或 component 2 事件的触发时间")
    
    total_events1 = sum(time_bin_counts1.values())
    total_events2 = sum(time_bin_counts2.values())
    print(f"\n总共处理了 {processed_file_count} 个文件")
    print(f"  Component 1: {total_events1} 个事件")
    print(f"  Component 2: {total_events2} 个事件")

    # 步骤4: 将时间bin计数转换为时间数组和计数数组
    print("\n步骤4: 整理时间bin数据...")
    
    # 合并所有时间bin
    all_bin_timestamps = set(time_bin_counts1.keys()) | set(time_bin_counts2.keys())
    
    if len(all_bin_timestamps) > 0:
        bin_times = [pd.Timestamp(ts, unit='ns') for ts in all_bin_timestamps]
        bin_counts1 = [time_bin_counts1.get(ts, 0) for ts in all_bin_timestamps]
        bin_counts2 = [time_bin_counts2.get(ts, 0) for ts in all_bin_timestamps]
        
        # 创建DataFrame并排序
        df_bins = pd.DataFrame({'count1': bin_counts1, 'count2': bin_counts2}, index=bin_times)
        df_bins = df_bins.sort_index()
        
        # 重新resample到指定的时间bin大小（确保bin对齐）
        resampled1 = df_bins['count1'].resample(time_bin_size).sum()
        resampled2 = df_bins['count2'].resample(time_bin_size).sum()
        
        bin_centers = resampled1.index.to_pydatetime()
        counts1 = resampled1.values.astype(int)
        counts2 = resampled2.values.astype(int)
    else:
        bin_centers = np.array([], dtype='datetime64[ns]')
        counts1 = np.array([], dtype=int)
        counts2 = np.array([], dtype=int)

    total_events1 = int(np.sum(counts1)) if len(counts1) > 0 else 0
    total_events2 = int(np.sum(counts2)) if len(counts2) > 0 else 0
    print(f"\n总共读取到:")
    print(f"  Component 1: {total_events1} 个事件（分布在 {len(bin_centers)} 个时间bin中）")
    print(f"  Component 2: {total_events2} 个事件（分布在 {len(bin_centers)} 个时间bin中）")
    if len(bin_centers) > 0:
        print(f"时间范围: {bin_centers[0]} 到 {bin_centers[-1]}")

    return bin_centers, counts1, counts2


def plot_component1_and_component2_countrate(
    hdf5_dir: Optional[str] = None,
    time_bin_size: str = "1h",
    ...
    """
    绘制GMM component 1和component 2事件计数率对比图
    ...
    """
    print("=" * 70)
    print("GMM component 1和component 2事件计数率对比绘制")
    print("=" * 70)

    # 1. 获取 component 1 和 component 2 事件计数率
    print("\n步骤1: 获取GMM component 1和component 2事件计数率...")
    bin_centers, counts1, counts2 = get_component1_and_component2_event_times(
        hdf5_dir, 
    ...
    # 绘制计数率（柱状图）
    width = pd.Timedelta(time_bin_size) * 0.35
    ax.bar(
        bin_centers - width/2,
        counts1,
        width=width,
    ...
    ax.set_ylabel("Event Count", fontsize=13, fontweight="normal")
    ax.legend(loc="upper left")

    # 格式化x轴日期
    bin_centers_dt64 = pd.to_datetime(bin_centers).values

    if len(bin_centers_dt64) > 1:
        time_span = bin_centers_dt64[-1] - bin_centers_dt64[0]
    ...
    # 计算统计信息
    count1_mean = np.mean(counts1) if len(counts1) > 0 else 0
    count1_std = np.std(counts1) if len(counts1) > 0 else 0
    count1_min = np.min(counts1) if len(counts1) > 0 else 0
    count1_max = np.max(counts1) if len(counts1) > 0 else 0
    total_events1 = np.sum(counts1)

    count2_mean = np.mean(counts2) if len(counts2) > 0 else 0
    count2_std = np.std(counts2) if len(counts2) > 0 else 0
    count2_min = np.min(counts2) if len(counts2) > 0 else 0
    count2_max = np.max(counts2) if len(counts2) > 0 else 0
    total_events2 = np.sum(counts2)

    # 添加统计信息框
    stats_text = (
        f"Component 1:\n"
        f"  Total = {total_events1}\n"
        f"  Mean = {count1_mean:.1f}\n"
        f"  Std = {count1_std:.1f}\n"
        f"  Min = {count1_min:.0f}\n"
        f"  Max = {count1_max:.0f}\n"
        f"\nComponent 2:\n"
        f"  Total = {total_events2}\n"
        f"  Mean = {count2_mean:.1f}\n"
        f"  Std = {count2_std:.1f}\n"
        f"  Min = {count2_min:.0f}\n"
        f"  Max = {count2_max:.0f}"
    )
    ...
    # 自动保存图片（如果save_path为None，自动生成保存路径）
    if save_path is None:
        ...
        if start_time:
            date_str += f"_{start_time.replace(':', '')}"
        if end_time:
            date_str += f"_{end_time.replace(':', '')}"
        
        filename = f"component1_component2_{date_str}.png"
...
if __name__ == "__main__":
    ...
    try:
        # 绘制
        plot_component1_and_component2_countrate(
            hdf5_dir=hdf5_dir,
