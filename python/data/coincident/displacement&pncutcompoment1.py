...
# -*- coding: utf-8 -*-
"""
振动位移数据和GMM pn cut component 1事件计数率联合绘制脚本
从振动传感器HDF5文件读取位移数据（5.28-6.10日），
...
# 导入振动位移模块
displacement_select_path = os.path.join(
    grandparent_dir, 'data', 'sensor', 'vibration', 'displacement', 'select.py'
)
spec_displacement = importlib.util.spec_from_file_location(
    "displacement_select", displacement_select_path
)
displacement_select = importlib.util.module_from_spec(spec_displacement)
spec_displacement.loader.exec_module(displacement_select)
select_by_date_range_vibration = displacement_select.select_by_date_range_vibration
...
def _process_single_file_for_component1_worker(args):
    """处理单个文件，返回component 1事件的时间bin计数（用于并行处理，必须在模块级别）"""
    (ch0_3_file, ch5_file, gmm_means, gmm_covariances, gmm_weights, gmm_precisions_cholesky,
     region2_label, a, b, sigma, start_datetime, end_datetime, time_bin_size, epoch_offset) = args
    ...
    try:
        ...
        if physical_count == 0:
            return (os.path.basename(ch0_3_file), {}, 0)
        ...
        with h5py.File(ch0_3_file, "r") as f:
            ...
            if len(file_event_datetimes) > 0:
                ...
                for bin_time, count in zip(resampled.index, resampled['count'].values):
                    ...
                    file_time_bin_counts[bin_timestamp] = int(count)
            
            return (os.path.basename(ch0_3_file), file_time_bin_counts, len(file_comp1_sel_indices))
...
def get_component1_event_times(
    ...
    if start_date is not None or end_date is not None:
        ...
        for ch0_3_file, ch5_file in matched_pairs:
            ...
            # 检查时间范围是否重叠
            # 重叠条件：not (file_end < start_datetime or file_start > end_datetime)
            # 即：file_end >= start_datetime and file_start <= end_datetime
            has_overlap = True
    ...
    # 步骤3: 并行处理所有文件，使用训练好的GMM模型筛选component 1事件
    print("\n步骤3: 并行处理所有文件，筛选component 1事件并累加到时间bin...")
    ...
    process_args = [
        (ch0_3_file, ch5_file, gmm_means, gmm_covariances, gmm_weights, gmm_precisions_cholesky,
         region2_label, a, b, sigma, start_datetime, end_datetime, time_bin_size, epoch_offset)
        for ch0_3_file, ch5_file in matched_pairs
    ]
    
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        futures = {executor.submit(_process_single_file_for_component1_worker, args): args[0] 
        ...
        for future in as_completed(futures):
            ...
            try:
                result_filename, file_time_bin_counts, comp1_count = future.result()
                
                # 累加到总计数
                for bin_timestamp, count in file_time_bin_counts.items():
                    if bin_timestamp not in time_bin_counts:
                        time_bin_counts[bin_timestamp] = 0
                    time_bin_counts[bin_timestamp] += count
                
                if comp1_count > 0:
                    processed_file_count += 1
                    print(f"  [{completed}/{len(matched_pairs)}] {result_filename}: "
                          f"找到 {comp1_count} 个 component 1 事件")
                else:
                    print(f"  [{completed}/{len(matched_pairs)}] {result_filename}: 跳过（无component 1事件）")
...
def plot_displacement_and_countrate(
    ...
    """
    print("=" * 70)
    print("振动位移数据和GMM component 1事件计数率联合绘制")
    print("=" * 70)
    ...
    if not vibration_data or "datetime" not in vibration_data:
        raise ValueError("未能读取到振动位移数据")

    vibration_datetime = vibration_data["datetime"]
    # 选择位移分量（优先选择 y 分量，如果没有则选择第一个可用的）
    displacement_columns = ["Displacement_y", "Displacement_z", "Displacement_x"]
    displacement_data = None
    displacement_name = None
    for col in displacement_columns:
        if col in vibration_data:
            displacement_data = vibration_data[col]
            displacement_name = col
            break

    if displacement_data is None:
        raise ValueError("未能找到位移数据列")

    # 过滤NaN值
    valid_mask = ~np.isnan(displacement_data)
    vibration_datetime_valid = vibration_datetime[valid_mask]
    displacement_data_valid = displacement_data[valid_mask]

    if len(vibration_datetime_valid) == 0:
        raise ValueError("振动位移数据中没有有效数据")

    print(f"读取到 {len(vibration_datetime_valid)} 个有效位移数据点")
    ...
    ax2.plot(
        ...
        displacement_data_valid,
        linewidth=1.5,
        alpha=0.85,
        color="#C73E1D",
        label=f"Displacement ({displacement_name})",
    )
    ax2.set_ylabel(
        f"Displacement ({displacement_name}) (μm)",
        fontsize=13,
        fontweight="normal",
        color="#C73E1D",
    )
    ...
    displacement_mean = np.mean(displacement_data_valid)
    displacement_std = np.std(displacement_data_valid)
    displacement_min = np.min(displacement_data_valid)
    displacement_max = np.max(displacement_data_valid)
    ...
    stats_text = (
        ...
        f"  Mean = {displacement_mean:.4f} μm\n"
        f"  Std = {displacement_std:.4f} μm\n"
        f"  Min = {displacement_min:.4f} μm\n"
        f"  Max = {displacement_max:.4f} μm"
    )
    ...
    if save_path is None:
        ...
        # current_file_dir = python/data/coincident
        ...
        # 生成文件名：displacement_y_component1_日期范围.png
        ...
        # 处理detector_num（可能是列表或单个数字）
        if isinstance(detector_num, (list, tuple)):
            det_str = "_".join(map(str, detector_num))
        else:
            det_str = str(detector_num)
        
        filename = f"displacement_y_component1_{date_str}_det{det_str}.png"
...
if __name__ == "__main__":
    ...
    try:
        # 绘制
        plot_displacement_and_countrate(
