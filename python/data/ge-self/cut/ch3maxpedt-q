...
# -*- coding: utf-8 -*-
"""
ch3maxpedt-q: 对通过 basic+act 全部 cut 的事例，从 CH0-3 读取 CH3 波形，
绘制横轴 max(CH3 尾部 500 点)、纵轴 q（CH3 全窗口积分值）的散点图。

调度策略：
- I/O 线程：预加载下一文件的 CH3 到内存，放入队列
- 计算：使用多线程对当前文件的 events 分块并行计算 max(pedt)/q（NumPy 释放 GIL）
...
def _load_passing_events_per_file() -> List[Tuple[Path, np.ndarray]]:
    ...
    print(f"[ch3maxpedt-q] 找到 {len(pairs)} 个可配对的参数文件。")
...
def _compute_max_q_parallel(
    ...
    """对 wf (n_samples, n_events) 分块并行计算 max(pedt) 和 q。"""
...
def main() -> None:
    ...
    if not file_tasks:
        print("[ch3maxpedt-q] 没有通过 cut 的事例可处理，退出。")
        return

    n_total = sum(len(idx) for _, idx in file_tasks)
    print(f"[ch3maxpedt-q] 共 {n_total} 个通过 cut 的事例，分布在 {len(file_tasks)} 个文件中。")

    mem = psutil.virtual_memory()
    avail_bytes = max(0, mem.total - RESERVE_MEMORY_BYTES)
    n_compute = _n_compute_workers()
    print(f"[ch3maxpedt-q] 可用内存约 {avail_bytes / 1024**3:.1f} GB，计算线程数 {n_compute}。")
    ...
    if not any(use for _, _, use in io_tasks) and io_tasks:
        print("[ch3maxpedt-q] 单文件 CH3 超可用内存，改用按事件读取。")
    ...
    while True:
        ...
        if item[0] is None and isinstance(item[1], Exception):
            print(f"[ch3maxpedt-q] I/O 加载失败: {item[1]}")
            continue
        wf, n_s = item
        if wf.size == 0:
            continue
        ch_max_pedt, q = _compute_max_q_parallel(wf, n_compute)
        all_max.append(ch_max_pedt)
        all_q.append(q)
        ...
        if fig is None:
            ...
            ax.set_xlabel(f"CH3 max(pedt), last {PEDT_TAIL_SAMPLES} samples (ADC)", fontsize=12)
            ax.set_ylabel("CH3 q (Σ signal×t, ADC·ns, 4ns/sample)", fontsize=12)
            ax.grid(True, alpha=0.3)
            scatter = ax.scatter([], [], s=2, alpha=0.5, edgecolors="none")
            fig.tight_layout()
            plt.show(block=False)
        x_vals = np.concatenate(all_max)
        ch3_q = np.concatenate(all_q)
        scatter.set_offsets(np.column_stack([x_vals, ch3_q]))
        ax.set_title(
            f"CH3 max(pedt) vs q (basic+act cuts passed, N={x_vals.size}, {received}/{n_files} )",
            fontsize=13,
        )
        ...
        if received % 10 == 0 or received == n_files:
            print(f"[ch3maxpedt-q] 进度: {received}/{n_files} 文件")

    io_thread.join(timeout=2.0)

    if not all_max:
        print("[ch3maxpedt-q] 未获取到任何 CH3 max(pedt)/q 数据，退出。")
        return

    x_vals = np.concatenate(all_max)
    ch3_q = np.concatenate(all_q)
    print(f"[ch3maxpedt-q] 共得到 {x_vals.size} 个 (max(pedt), q) 数据点。")
    plt.ioff()
    plt.show()
