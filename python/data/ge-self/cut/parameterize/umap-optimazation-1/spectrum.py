...
# -*- coding: utf-8 -*-
"""
从 15 参数 UMAP+HDBSCAN 事件映射 HDF5 中，选出指定 cluster 的所有事件，
...
HDF5 结构参考 15parameter_spectrum.py 中的 `_save_event_mapping_hdf5`：
...
def _compute_max_ch0_for_cluster(
    ...
    """
    对给定 cluster 中的所有事件，从 CH0max 文件中读取 max(ch0)；
    ...
    script_dir = Path(__file__).resolve().parent  # .../parameterize/umap-optimazation-1
    ...
    for fi, ev_list in file_to_events.items():
        ...
        # CH0max 文件（提供所有事件的 max_ch0）
        ch0max_path = ch0max_dir / src_path.name
        if not ch0max_path.exists():
            print(f"警告: CH0max 文件不存在，跳过: {ch0max_path}")
            continue

        ev_arr = np.asarray(ev_list, dtype=np.int64)

        with h5py.File(ch0max_path, "r") as f_max:
            if "max_ch0" not in f_max:
                print(f"警告: CH0max 文件中缺少 'max_ch0' 数据集，跳过: {ch0max_path}")
                continue
            dset = f_max["max_ch0"]
            if dset.ndim != 1:
            ...
            max_vals_file = np.asarray(dset[ev_arr], dtype=np.float64)
...
def _default_hdf5_path_relative_to_script() -> Path:
    ...
    这里默认使用与 clustter.py 相同的事件映射文件名。
    """
    script_dir = Path(__file__).resolve().parent  # .../cut/parameterize/umap-optimazation-1
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "读取 15 参数 UMAP+HDBSCAN 事件映射 HDF5，"
