...
def _discover_project_root() -> Path:
    ...
    # 5: python
    # 6: DeepVibration
    python_dir = here.parents[6]           # .../python
...
# UMAP+HDBSCAN 结果缓存默认保存路径（事件映射 HDF5），保存至本脚本同目录
_SCRIPT_DIR = Path(__file__).resolve().parent
CLUSTER_CACHE_PATH = _SCRIPT_DIR / "geact_30param_umap_hdbscan_eventmap.h5"
...
def main() -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "ch3_ch3ped_mean / ch3_min_ch3 之一，"
    ...
    ch3ped_mean_all = X[:, idx_ch3_ped]
    min_ch3_all = X[:, idx_ch3_min]

    # 分别应用基础 cut
    m_ch0_min = cut_ch0_min_positive(ch0_min_all)
    m_ch0_max = cut_ch0_max_saturation(ch0_max_all, max_ch1_all)
    m_ch5_rt = cut_ch5_self_trigger(ch5_max_all)
    m_acv = cut_acv(max_ch4_all, tmax_ch4_all)
    m_ch3_ped_min = cut_ch3ped_min(ch3ped_mean_all, min_ch3_all)
    # 先得到基础 mask，
    base_mask = m_fit_ok & m_ch0_min & m_ch0_max & m_ch5_rt & ~m_acv & m_ch3_ped_min
    ...
    print(
        f"根据物理 cuts"
        f"筛选后剩余事件数: {n_kept} / {n_total_events}"
    )
    if n_kept == 0:
