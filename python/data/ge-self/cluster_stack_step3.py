...
# -*- coding: utf-8 -*-
"""
针对 30parameter&HDBSCAN_step3.py 写出的事件映射 HDF5，文件名规则为：

    ge_30param_umap_hdbscan_eventmap_step2_cluster{Y}_step_3_cluster{Z}.h5

其中 Y 对应 step2 的父簇（第一层子集），Z 对应 step3 在 step2 文件内所选的源簇
（与 step3 脚本中 --step2-cluster 一致）。

默认由 --step2-parent、--step3-source 拼出上述路径；也可直接传入 HDF5 路径。
按 **--cluster** 指定的子簇（event_cluster_labels，可多个）依次绘制，流程与
cluster_stack 一致：每个 cluster 单独读波形、单独弹窗，**默认 CH3**。
...
# CH3 在 preprocessor 保存的 channel_data 中为索引 3（CH0=0, CH1=1, ...）
DEFAULT_CHANNEL_INDEX = 3

# 与 cluster_stack.py、cluster_stack_step2.py、python/utils/plotstyle.md 一致
_PLOT_TICK = 12
_PLOT_AXIS = 16
_PLOT_TITLE = 18
_PLOT_LEGEND = 12
...
def step3_eventmap_filename(step2_parent: int, step3_source: int) -> str:
    """
    与 30parameter&HDBSCAN_step3.py 输出一致：
    ge_30param_umap_hdbscan_eventmap_step2_cluster{Y}_step_3_cluster{Z}.h5
    """
    return (
        f"ge_30param_umap_hdbscan_eventmap_step2_cluster{int(step2_parent)}"
        f"_step_3_cluster{int(step3_source)}.h5"
    )


def default_step3_eventmap_path(step2_parent: int, step3_source: int) -> Path:
    return DEFAULT_HDF5_DIR / step3_eventmap_filename(step2_parent, step3_source)
...
def plot_step3_cluster_stack(
    ...
    `cluster_labels` 为 step3 eventmap 内 event_cluster_labels 中要绘制的标签（可含 -1）。
    """
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "step3 eventmap（ge_*_step2_cluster{Y}_step_3_cluster{Z}.h5）："
            "指定 eventmap 内子 cluster，与 cluster_stack 相同方式读波形并弹窗（默认 CH3）。"
        ),
    )
    p.add_argument(
        "hdf5_path",
        ...
        default=None,
        help=(
            "step3 写出的事件映射 HDF5 完整路径；若省略则用 --step2-parent / --step3-source "
            "拼出 data/hdf5/ge_30param_umap_hdbscan_eventmap_step2_cluster{Y}_step_3_cluster{Z}.h5。"
        ),
    )
    p.add_argument(
        "--step2-parent",
        ...
        default=1,
        metavar="Y",
        help="文件名中 _step2_cluster 后的编号 Y（仅当未传 hdf5_path 时使用）。",
    )
    p.add_argument(
        "--step3-source",
        type=int,
        default=2,
        metavar="Z",
        help="文件名中 _step_3_cluster 后的编号 Z，与 step3 脚本 --step2-cluster 一致（仅当未传 hdf5_path 时使用）。",
    )
    p.add_argument(
        "--cluster",
        type=int,
        ...
        metavar="LABEL",
        help=(
            "该 step3 HDF5 内 event_cluster_labels 中要绘制的子 cluster，可多个，如 --cluster 0 1 2；"
            "默认 [2]。噪声为 -1。单独写 --cluster 无参数则回退为 [0]。"
        ),
    )
    ...
    p.add_argument(
        ...
        help="channel_data 通道索引（默认 3=CH3）。",
    )
...
def main() -> None:
    ...
    if args.hdf5_path:
        ...
        mapping_path = default_step3_eventmap_path(
            int(args.step2_parent),
            int(args.step3_source),
        ).resolve()
    print(f"[信息] 使用 eventmap: {mapping_path}")

    plot_step3_cluster_stack(
