...
def select_by_date_range_vibration(data_dir: str,
    ...
    print(f'\n开始读取 {len(detector_list)} 个探测器的频率数据: {detector_list}')
    if downsample_factor > 1:
...
def plot_frequency_data(data_dict: Dict[str, np.ndarray],
                       save_path: Optional[str] = None,
                       show_plot: bool = True,
                       figsize: Tuple[int, int] = (14, 8)) -> None:
    """
    绘制频率数据随时间的变化
    如果数据中包含多个探测器（detector_num列），会为每个频率分量创建子图，每个子图显示所有探测器
    
    参数:
        data_dict: 包含频率数据的字典，必须包含 'datetime' 和频率列
    ...
    frequency_columns = ['Frequency_x', 'Frequency_y', 'Frequency_z']
    
    # 检查是否有频率数据
    available_freq_cols = [col for col in frequency_columns if col in data_dict]
    if not available_freq_cols:
        print('警告：没有找到频率数据列，无法绘图')
        return
    ...
    # 创建图形和子图 - 为每个频率分量创建子图
    fig, axes = plt.subplots(len(available_freq_cols), 1, figsize=figsize, sharex=True)
    if len(available_freq_cols) == 1:
        axes = [axes]
    ...
    for ax_idx, freq_col in enumerate(available_freq_cols):
        ...
        if has_multiple_detectors:
            ...
            for i, det_num in enumerate(sorted(unique_detectors)):
                ...
                det_freq = freq_data[det_mask]
                
                # 过滤NaN值
                valid_mask = ~np.isnan(det_freq)
                det_datetime_valid = det_datetime[valid_mask]
                det_freq_valid = det_freq[valid_mask]
                
                if len(det_datetime_valid) > 0:
                    color = colors[i % len(colors)]
                    ax.plot(det_datetime_valid, det_freq_valid,
                           linewidth=1.5,
                           alpha=0.85,
                           color=color,
            ...
            freq_valid = freq_data[valid_mask]
            
            if len(datetime_valid) > 0:
                color = freq_colors.get(freq_col, colors[0])
                label = freq_col
                ax.plot(datetime_valid, freq_valid,
                       linewidth=1.5,
                       alpha=0.85,
                       color=color,
                       label=label)
...
if __name__ == '__main__':
    # 获取项目根目录
    # 从 python/data/sensor/vibration/frequency/select.py 向上5层到达项目根目录
    # frequency -> vibration -> sensor -> data -> python -> DeepVibration (项目根目录)
    current_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(current_dir)))))
    data_dir = os.path.join(project_root, 'data', 'vibration', 'hdf5')
    
    print('=' * 70)
    print('振动传感器频率数据读取和筛选示例')
    print('=' * 70)
    ...
    try:
        ...
        if data_multi:
            ...
            print('\n频率数据统计信息:')
            print('-' * 70)
            for freq_col in ['Frequency_x', 'Frequency_y', 'Frequency_z']:
                if freq_col in data_multi:
                    freq_data = data_multi[freq_col]
                    valid_data = freq_data[~np.isnan(freq_data)]
                    if len(valid_data) > 0:
                        print(f'{freq_col}:')
                        ...
                        print(f'{freq_col}: 无有效数据')
            
            # 绘制频率数据图像
            print('\n开始绘制频率数据图像...')
            plot_frequency_data(data_multi, save_path=None, show_plot=True)
        else:
            print('警告：未能读取到任何数据')
