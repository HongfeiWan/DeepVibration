...
"""
振动传感器位移数据读取和筛选脚本
从detector_{num}_YYYY-MM-DD.h5文件中读取数据，并按日期范围筛选
...
def read_vibration_data(file_path: str, downsample_factor: int = 1) -> pd.DataFrame:
    ...
    try:
        with h5py.File(file_path, 'r') as h5f:
            # 检查必需的数据集是否存在
            required_datasets = ['time', 'datetime_str']
            ...
            for col_name in displacement_columns:
                if col_name in h5f:
                    if downsample_factor > 1:
...
def select_by_date_range_vibration(data_dir: str,
    ...
    print(f'\n开始读取 {len(detector_list)} 个探测器的位移数据: {detector_list}')
    if downsample_factor > 1:
...
def plot_displacement_data(data_dict: Dict[str, np.ndarray],
                       save_path: Optional[str] = None,
                       show_plot: bool = True,
                       figsize: Tuple[int, int] = (14, 8)) -> None:
    """
    绘制位移数据随时间的变化
    如果数据中包含多个探测器（detector_num列），会为每个位移分量创建子图，每个子图显示所有探测器
    
    参数:
        data_dict: 包含位移数据的字典，必须包含 'datetime' 和位移列
                  如果包含 'detector_num' 列，会按探测器分组绘制多条曲线
        save_path: 保存图像的路径（可选），如果为None则不保存
        show_plot: 是否显示图像，默认为True
        figsize: 图像大小，默认为(14, 8)
    """
    if not data_dict or 'datetime' not in data_dict:
        print('警告：数据字典为空或缺少datetime列，无法绘图')
        return
    
    datetime_arr = data_dict['datetime']
    displacement_columns = ['Displacement_x', 'Displacement_y', 'Displacement_z']
    
    # 检查是否有位移数据
    available_disp_cols = [col for col in displacement_columns if col in data_dict]
    if not available_disp_cols:
        print('警告：没有找到位移数据列，无法绘图')
        return
    ...
    if has_multiple_detectors:
        ...
        has_multiple_detectors = len(unique_detectors) > 1
    else:
    ...
    disp_colors = {'Displacement_x': '#2E86AB', 'Displacement_y': '#A23B72', 'Displacement_z': '#F18F01'}
    
    # 创建图形和子图 - 为每个位移分量创建子图
    fig, axes = plt.subplots(len(available_disp_cols), 1, figsize=figsize, sharex=True)
    if len(available_disp_cols) == 1:
        axes = [axes]
    
    # 获取时间范围用于日期格式化
    all_datetime_valid = []
    for disp_col in available_disp_cols:
        disp_data = data_dict[disp_col]
        if has_multiple_detectors:
            valid_mask = ~np.isnan(disp_data)
            all_datetime_valid.extend(datetime_arr[valid_mask])
        else:
            valid_mask = ~np.isnan(disp_data)
            all_datetime_valid.extend(datetime_arr[valid_mask])
    
    if len(all_datetime_valid) == 0:
    ...
    # 为每个位移分量绘制子图
    for ax_idx, disp_col in enumerate(available_disp_cols):
        ax = axes[ax_idx]
        disp_data = data_dict[disp_col]
        ...
        if has_multiple_detectors:
            ...
            for i, det_num in enumerate(sorted(unique_detectors)):
                ...
                det_disp = disp_data[det_mask]
                
                # 过滤NaN值
                valid_mask = ~np.isnan(det_disp)
                det_datetime_valid = det_datetime[valid_mask]
                det_disp_valid = det_disp[valid_mask]
                
                if len(det_datetime_valid) > 0:
                    color = colors[i % len(colors)]
                    ax.plot(det_datetime_valid, det_disp_valid,
                           linewidth=1.5,
                           alpha=0.85,
                           color=color,
                    ...
                    # 计算统计信息
                    mean_val = np.mean(det_disp_valid)
                    std_val = np.std(det_disp_valid)
                    min_val = np.min(det_disp_valid)
                    max_val = np.max(det_disp_valid)
                    all_stats.append((det_num, len(det_disp_valid), min_val, max_val, mean_val, std_val))
        else:
            # 单个探测器：绘制单条曲线
            valid_mask = ~np.isnan(disp_data)
            datetime_valid = datetime_arr[valid_mask]
            disp_valid = disp_data[valid_mask]
            
            if len(datetime_valid) > 0:
                color = disp_colors.get(disp_col, colors[0])
                label = disp_col
                ax.plot(datetime_valid, disp_valid,
                       linewidth=1.5,
                       alpha=0.85,
                       color=color,
                       label=label)
                ...
                # 计算统计信息
                mean_val = np.mean(disp_valid)
                std_val = np.std(disp_valid)
                min_val = np.min(disp_valid)
                max_val = np.max(disp_valid)
                all_stats.append((disp_col, len(disp_valid), min_val, max_val, mean_val, std_val))
        
        # 设置标签
        ax.set_ylabel(f'{disp_col} (μm)', fontsize=13, fontweight='normal')
        if ax_idx == len(available_disp_cols) - 1:
        ...
        if len(all_stats) > 0:
            ...
            for _, _, min_val, max_val, _, _ in all_stats:
                all_disp_vals.extend([min_val, max_val])
            disp_range = np.max(all_disp_vals) - np.min(all_disp_vals)
            if disp_range > 0:
                minor_interval = disp_range / 20
            ...
            for stat_item in all_stats:
                if has_multiple_detectors:
                    ...
                    stats_text += (f'Det {det_num}: N={n}, '
                                 f'Mean={mean_val:.2f} μm, '
                                 f'Std={std_val:.2f} μm\n')
                else:
                    disp_name, n, min_val, max_val, mean_val, std_val = stat_item
                    stats_text += (f'N={n}, '
                                 f'Mean={mean_val:.2f} μm, '
                                 f'Std={std_val:.2f} μm\n')
...
if __name__ == '__main__':
    # 获取项目根目录
    # 从 python/data/sensor/vibration/displacement/select.py 向上5层到达项目根目录
    # displacement -> vibration -> sensor -> data -> python -> DeepVibration (项目根目录)
    current_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(current_dir)))))
    data_dir = os.path.join(project_root, 'data', 'vibration', 'hdf5')
    
    print('=' * 70)
    print('振动传感器位移数据读取和筛选示例')
    print('=' * 70)
    ...
    try:
        ...
        if data_multi:
            ...
            print('\n位移数据统计信息:')
            print('-' * 70)
            for disp_col in ['Displacement_x', 'Displacement_y', 'Displacement_z']:
                if disp_col in data_multi:
                    disp_data = data_multi[disp_col]
                    valid_data = disp_data[~np.isnan(disp_data)]
                    if len(valid_data) > 0:
                        print(f'{disp_col}:')
                        ...
                        print(f'{disp_col}: 无有效数据')
            
            # 绘制位移数据图像
            print('\n开始绘制位移数据图像...')
            plot_displacement_data(data_multi, save_path=None, show_plot=True)
        else:
            print('警告：未能读取到任何数据')
