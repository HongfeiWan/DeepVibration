...
def select_by_date_range_vibration(data_dir: str,
    ...
    print(f'\n开始读取 {len(detector_list)} 个探测器的速度数据: {detector_list}')
    if downsample_factor > 1:
...
def plot_velocity_data(data_dict: Dict[str, np.ndarray],
                       save_path: Optional[str] = None,
                       show_plot: bool = True,
                       figsize: Tuple[int, int] = (14, 8)) -> None:
    """
    绘制速度数据随时间的变化
    如果数据中包含多个探测器（detector_num列），会为每个速度分量创建子图，每个子图显示所有探测器
    
    参数:
        data_dict: 包含速度数据的字典，必须包含 'datetime' 和速度列
    ...
    # 检查是否有速度数据
    available_vel_cols = [col for col in velocity_columns if col in data_dict]
    if not available_vel_cols:
        print('警告：没有找到速度数据列，无法绘图')
        return
    ...
    vel_colors = {'Velocity_x': '#2E86AB', 'Velocity_y': '#A23B72', 'Velocity_z': '#F18F01'}
    
    # 创建图形和子图 - 为每个速度分量创建子图
    fig, axes = plt.subplots(len(available_vel_cols), 1, figsize=figsize, sharex=True)
    if len(available_vel_cols) == 1:
        axes = [axes]
    
    # 获取时间范围用于日期格式化
    all_datetime_valid = []
    for vel_col in available_vel_cols:
        vel_data = data_dict[vel_col]
        if has_multiple_detectors:
    ...
    # 为每个速度分量绘制子图
    for ax_idx, vel_col in enumerate(available_vel_cols):
        ax = axes[ax_idx]
        vel_data = data_dict[vel_col]
        ...
        if has_multiple_detectors:
            ...
            for i, det_num in enumerate(sorted(unique_detectors)):
                ...
                det_vel = vel_data[det_mask]
                
                # 过滤NaN值
                valid_mask = ~np.isnan(det_vel)
                det_datetime_valid = det_datetime[valid_mask]
                det_vel_valid = det_vel[valid_mask]
                
                if len(det_datetime_valid) > 0:
                    color = colors[i % len(colors)]
                    ax.plot(det_datetime_valid, det_vel_valid,
                           linewidth=1.5,
                           alpha=0.85,
                           color=color,
                    ...
                    # 计算统计信息
                    mean_val = np.mean(det_vel_valid)
                    std_val = np.std(det_vel_valid)
                    min_val = np.min(det_vel_valid)
                    max_val = np.max(det_vel_valid)
                    all_stats.append((det_num, len(det_vel_valid), min_val, max_val, mean_val, std_val))
        else:
            # 单个探测器：绘制单条曲线
            valid_mask = ~np.isnan(vel_data)
            ...
            if len(datetime_valid) > 0:
                color = vel_colors.get(vel_col, colors[0])
                label = vel_col
                ax.plot(datetime_valid, vel_valid,
                       linewidth=1.5,
                       alpha=0.85,
                       color=color,
                       label=label)
                ...
                # 计算统计信息
                mean_val = np.mean(vel_valid)
                std_val = np.std(vel_valid)
                min_val = np.min(vel_valid)
                max_val = np.max(vel_valid)
                all_stats.append((vel_col, len(vel_valid), min_val, max_val, mean_val, std_val))
        
        # 设置标签
        ax.set_ylabel(f'{vel_col} (mm/s)', fontsize=13, fontweight='normal')
        if ax_idx == len(available_vel_cols) - 1:
        ...
        if len(all_stats) > 0:
            ...
            for _, _, min_val, max_val, _, _ in all_stats:
                all_vel_vals.extend([min_val, max_val])
            vel_range = np.max(all_vel_vals) - np.min(all_vel_vals)
            if vel_range > 0:
                minor_interval = vel_range / 20
            ...
            for stat_item in all_stats:
                if has_multiple_detectors:
                    ...
                    stats_text += (f'Det {det_num}: N={n}, '
                                 f'Mean={mean_val:.2f} mm/s, '
                                 f'Std={std_val:.2f} mm/s\n')
                else:
                    vel_name, n, min_val, max_val, mean_val, std_val = stat_item
                    stats_text += (f'N={n}, '
                                 f'Mean={mean_val:.2f} mm/s, '
                                 f'Std={std_val:.2f} mm/s\n')
...
if __name__ == '__main__':
    # 获取项目根目录
    # 从 python/data/sensor/vibration/velocity/select.py 向上5层到达项目根目录
    # velocity -> vibration -> sensor -> data -> python -> DeepVibration (项目根目录)
    current_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(current_dir)))))
    data_dir = os.path.join(project_root, 'data', 'vibration', 'hdf5')
    
    print('=' * 70)
    print('振动传感器速度数据读取和筛选示例')
    print('=' * 70)
    ...
    try:
        ...
        if data_multi:
            ...
            for vel_col in ['Velocity_x', 'Velocity_y', 'Velocity_z']:
                if vel_col in data_multi:
                    vel_data = data_multi[vel_col]
                    valid_data = vel_data[~np.isnan(vel_data)]
                    if len(valid_data) > 0:
                        print(f'{vel_col}:')
                        ...
                        print(f'{vel_col}: 无有效数据')
            
            # 绘制速度数据图像
            print('\n开始绘制速度数据图像...')
            plot_velocity_data(data_multi, save_path=None, show_plot=True)
        else:
            print('警告：未能读取到任何数据')
