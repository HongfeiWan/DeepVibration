...
def plot_environment_overview(
    ...
    # 动态导入各模块
    temp_mod = _load_module(
        "vib_temp_select",
        os.path.join(project_root, "python", "data", "sensor", "vibration", "temperature", "select.py"),
    )
    acc_mod = _load_module(
        "vib_acc_select",
        os.path.join(project_root, "python", "data", "sensor", "vibration", "accelerate", "select.py"),
    )
    vel_mod = _load_module(
        "vib_vel_select",
        os.path.join(project_root, "python", "data", "sensor", "vibration", "velocity", "select.py"),
    )
    disp_mod = _load_module(
        "vib_disp_select",
        os.path.join(project_root, "python", "data", "sensor", "vibration", "displacement", "select.py"),
    )
    freq_mod = _load_module(
        "vib_freq_select",
        os.path.join(project_root, "python", "data", "sensor", "vibration", "frequency", "select.py"),
    )
    compressor_mod = _load_module(
        "compressor_select",
        os.path.join(project_root, "python", "data", "compressor", "select.py"),
    )
    ...
    temp_data = temp_mod.select_by_date_range_vibration(**common_kwargs)
    acc_data = acc_mod.select_by_date_range_vibration(**common_kwargs)
    vel_data = vel_mod.select_by_date_range_vibration(**common_kwargs)
    disp_data = disp_mod.select_by_date_range_vibration(**common_kwargs)
    freq_data = freq_mod.select_by_date_range_vibration(**common_kwargs)

    compressor_data = compressor_mod.select_by_date_range(
