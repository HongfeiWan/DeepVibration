% 处理bin文件并提取第6个通道的Random trigger原始波形数据
% 修改说明：只保留第6个通道的数据，不保存其他通道信息

% 定义文件路径和文件名
read_path = '/st0/share/data/raw_data/CEvNS_DZL_test_sanmen/20250520/';
save_path = '/st0/home/wanhf/RT';
...
% 定义通道数和事件数
CHANNEL_NUMBER = 1; % 只处理第6个通道
TARGET_CHANNEL = 6; % 目标通道编号（Random trigger通道）
...
if use_parallel
    parfor i = RUN_Start_NUMBER:RUN_End_NUMBER
        ...
        try
            process_single_file_optimized(run_filename, CHANNEL_NUMBER, TARGET_CHANNEL, EVENT_NUMBER, SAVE_OPTIONS, save_path);
        catch ME
...
% 定义优化的处理单个文件的函数
function process_single_file_optimized(run_filename, CHANNEL_NUMBER, TARGET_CHANNEL, EVENT_NUMBER, SAVE_OPTIONS, save_path)
    fprintf('============================================\n');
    fprintf('Opening %s\n', run_filename);
    ...
    for j = 1:EVENT_NUMBER
        ...
        for k = 1:16 % 仍然需要读取所有16个通道的数据
            ...
            % 只保存第6个通道的数据
            if k == TARGET_CHANNEL && ~SAVE_OPTIONS.save_only_processed
                channel_data(:, 1, j) = channel_data_temp; % 存储到第1个位置
            end
        end
    ...
    for j = 1:EVENT_NUMBER
        ...
        if SAVE_OPTIONS.save_only_processed
            ...
            V1725_1_pulse = channel_data(:, 1, j); % 第6个通道的数据存储在第1个位置
        end
    ...
    if ~SAVE_OPTIONS.save_only_processed
        ...
        fprintf('估算文件大小: %.2f GB (仅第%d通道数据)\n', estimated_size_GB, TARGET_CHANNEL);
    ...
    if SAVE_OPTIONS.save_in_batches
        ...
        % 一次性保存
        if ~SAVE_OPTIONS.save_only_processed && ~isempty(channel_data)
            % 保存所有数据 - 使用v7.3格式，确保数据完整性
            try
                if SAVE_OPTIONS.use_compression
                ...
                % 尝试保存为更兼容的格式
                try
                    save(sprintf('%s.mat', base_filename), 'channel_data', 'time_data', '-v7');
                    fprintf('使用v7格式保存成功，耗时: %.2f秒\n', toc);
                catch ME2
                    fprintf('v7格式保存也失败: %s\n', ME2.message);
                    % 最后尝试只保存时间数据
                    save(sprintf('%s_time_only.mat', base_filename), 'time_data', '-v7');
                    fprintf('仅保存时间数据成功\n');
                end
            end
        else
            ...
            try
                ...
                fprintf('时间数据保存失败: %s，尝试v7格式\n', ME.message);
                save(sprintf('%s_time_only.mat', base_filename), 'time_data', '-v7');
                fprintf('使用v7格式保存时间数据成功\n');
            end
        end
    end
